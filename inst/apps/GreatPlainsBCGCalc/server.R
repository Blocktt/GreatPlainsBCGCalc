  ...
  observeEvent(input$b_calc_bcg, {
    shiny::withProgress({
      ...
      if (my_comm == "Bugs_IA") {
        ...
        msg <- paste0("Data, Qual, dim = ",
                      paste(dim(df_input_lr_t), collapse = ", "))
        message(msg)
        msg <- paste0("Data, Quant, dim = ",
                      paste(dim(df_input_lr_f), collapse = ", "))
        message(msg)
        # calc
        if (nrow(df_input_lr_t) == 0) {
          ...
          df_metval_lr_f <- BioMonTools::metric.values(df_input_lr_f
                                          ...
                                          , taxaid_dni = "DNI")#|>
            # # filter
            # dplyr::select(dplyr::any_of(c("SAMPLEID",
            #                               "INDEX_NAME",
            #                               "INDEX_CLASS",
            #                               metnames_lr_f)))
